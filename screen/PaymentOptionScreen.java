...
public class PaymentOptionScreen extends JFrame {
    ...
    public PaymentOptionScreen(AppController controller) {
        ...
        title.setFont(new Font("공요의 그리글", Font.BOLD, 36));
    ...
    private JButton createRoundedButton(String text, String fileName) {
        ...
        // 버튼 스타일 지정함
        btn.setFont(new Font("공요의 그리글", Font.BOLD, 18));
    ...
    private void openReceiptOptionScreen() {
        ...
        thanksLabel.setFont(new Font("공요의 그리글", Font.BOLD, 26));
        ...
        msg.setFont(new Font("공요의 그리글", Font.PLAIN, 20));
        ...
        for (JButton b : new JButton[]{bothBtn, orderOnlyBtn}) {
            b.setFont(new Font("공요의 그리글", Font.BOLD, 20));
    ...
    private void showConfirmation(String paymentMethod, String receiptOption) {
        ...
        List<CartItem> items = controller.getCart().getItems();
        ...
        for (CartItem item : items) {
            ...
            int subtotal = item.getItemPrice();
            total += subtotal;

            sb.append("- ").append(name).append(" x").append(qty);

            String opt = item.optionSummary();
            if (opt != null && !opt.isBlank()) sb.append(" [").append(opt).append("]");

            sb.append(" (").append(subtotal).append("원)\n");
        }

        sb.append("\n총 금액: ").append(total).append("원\n")
          .append("결제수단: ").append(paymentMethod).append("\n")
          .append(receiptOption).append("합니다.");

        JOptionPane.showMessageDialog(this, "주문번호: " + orderNumber + "\n\n" + sb.toString());

        controller.newScreen(); // 다시 초기 화면으로 이동함
        controller.resetCart(); // 카트 초기화함
    }
    ...
    class MarqueePanel extends JPanel {
        ...
        public MarqueePanel(String text, int speed) {
            ...
            setFont(new Font("공요의 그리글", Font.BOLD, 20));
